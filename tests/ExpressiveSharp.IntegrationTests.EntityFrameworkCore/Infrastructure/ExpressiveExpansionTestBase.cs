...
/// Subclasses provide a configured <see cref="IntegrationTestDbContext"/> via
/// <see cref="CreateContext"/> for the target database provider.
/// </summary>
public abstract class ExpressiveExpansionTestBase
{
    protected IntegrationTestDbContext Context { get; private set; } = null!;

    protected abstract IntegrationTestDbContext CreateContext();
    ...
    public async Task Setup()
    {
        ...
        foreach (var c in SeedData.Customers)
            Context.Customers.Add(new Customer { Id = c.Id, Name = c.Name, Email = c.Email, AddressId = c.AddressId });
        await Context.SaveChangesAsync();
        foreach (var o in SeedData.Orders)
            Context.Orders.Add(new Order { Id = o.Id, Tag = o.Tag, Price = o.Price, Quantity = o.Quantity, Status = o.Status, CustomerId = o.CustomerId });
        await Context.SaveChangesAsync();
