...
- `src/ExpressiveSharp/IOrderedExpressiveQueryable.cs` — zero-member marker bridging `IOrderedQueryable<T>` and `IExpressiveQueryable<T>`, returned by `OrderBy` and required by `ThenBy` (matching `System.Linq.Queryable`). Deliberately **no** ordered × includable cross-product: `Include`/`AsNoTracking` must precede the ordering. `AsExpressive` has an `IOrderedQueryable<T>` overload so ordering interceptors re-wrap with no generator-side factory selection
