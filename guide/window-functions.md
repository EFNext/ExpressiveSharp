---
url: 'https://efnext.github.io/ExpressiveSharp/guide/window-functions.md'
---
...
Navigation functions access specific rows relative to the current row. LAG/LEAD do not support frame clauses; FIRST\_VALUE/LAST\_VALUE do.
...
::: warning LAST\_VALUE needs an explicit frame
...
The SQL standard forbids frame clauses on ranking functions (ROW\_NUMBER, RANK, DENSE\_RANK, NTILE) — SQL Server and PostgreSQL will reject the query. Aggregate functions (SUM, AVG, COUNT, MIN, MAX) and value functions (FIRST\_VALUE, LAST\_VALUE, NTH\_VALUE) accept frames.
:::
