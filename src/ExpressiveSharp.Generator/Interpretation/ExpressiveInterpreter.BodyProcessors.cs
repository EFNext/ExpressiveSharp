...
static internal partial class ExpressiveInterpreter
{
    ...
    /// <summary>
    /// Fills <paramref name="descriptor"/> from a <c>[Expressive(Projectable = true)]</c>
    /// property. The get accessor must have the pattern <c>field ?? (&lt;formula&gt;)</c>, and the
    /// init/set accessor must be a plain <c>field = value</c> assignment. The formula (the right
    /// operand of the coalesce) is extracted and fed through the regular property-body emission
    /// pipeline, so the registry records a lambda keyed on the property's getter handle with the
    /// formula as its body. Returns <c>false</c> and reports diagnostics on failure.
    /// </summary>
    private static bool TryApplyProjectablePropertyBody(
        PropertyDeclarationSyntax propertyDeclarationSyntax,
        ISymbol memberSymbol,
        SemanticModel semanticModel,
        DeclarationSyntaxRewriter declarationSyntaxRewriter,
        ...
        ExpressiveDescriptor descriptor)
    {
        var propertyLocation = propertyDeclarationSyntax.Identifier.GetLocation();

        if (memberSymbol is not IPropertySymbol propertySymbol)
        {
            // Belt-and-braces — the dispatch is already property-specific.
            return false;
        }

        // ── Step 1: Property-level validation ───────────────────────────────

        // EXP0028: not allowed on interface members (interfaces cannot have instance fields).
        if (propertySymbol.ContainingType.TypeKind == TypeKind.Interface)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableNotAllowedOnInterface, propertyLocation));
            return false;
        }

        // EXP0029: not allowed on override properties.
        if (propertySymbol.IsOverride)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableNotAllowedOnOverride, propertyLocation));
            return false;
        }

        // EXP0022 (specialized): abstract or extern has no body to analyze.
        if (propertySymbol.IsAbstract || propertySymbol.IsExtern)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableGetAccessorPattern, propertyLocation,
                "abstract or extern property has no body to analyze"));
            return false;
        }

        // EXP0021: must have a writable accessor (set or init).
        if (propertySymbol.SetMethod is null)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableRequiresWritableAccessor, propertyLocation,
                propertySymbol.Name));
            return false;
        }
        ...
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableIncompatibleWithRequired, propertyLocation,
                propertySymbol.Name));
            return false;
        }

        // ── Step 2: Find the get and set/init accessor declarations ─────────

        if (propertyDeclarationSyntax.AccessorList is null)
        {
            // Expression-bodied property (=> expr) has no accessor list, so it cannot have an
            // `init` or `set` accessor. This shape is rejected by the writable-accessor check
            // above, but be defensive.
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableGetAccessorPattern, propertyLocation,
                "Projectable properties must use a { get; init/set; } accessor list, not expression-body syntax"));
            return false;
        }

        var getAccessor = propertyDeclarationSyntax.AccessorList.Accessors
            .FirstOrDefault(a => a.IsKind(SyntaxKind.GetAccessorDeclaration));
        var setAccessor = propertyDeclarationSyntax.AccessorList.Accessors
        ...
        if (getAccessor is null)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableGetAccessorPattern, propertyLocation,
                "Projectable property must declare a get accessor"));
            return false;
        }
        ...
        // ── Step 3: Recognize the get accessor pattern ──────────────────────

        var getOperation = semanticModel.GetOperation(getAccessor);
        if (getOperation is null)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableGetAccessorPattern, getAccessor.GetLocation(),
                "get accessor has no analyzable body"));
            return false;
        }
        ...
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableSetterMustStoreToBackingField, setAccessor.GetLocation(),
        ...
        if (formulaSyntax is null)
        {
            context.ReportDiagnostic(Diagnostic.Create(
                Diagnostics.ProjectableGetAccessorPattern, getAccessor.GetLocation(),
                "formula expression has no source syntax"));
            return false;
        }
        ...
        descriptor.ExpressionTreeEmission = EmitExpressionTreeForProperty(
            formulaSyntax, semanticModel, context, descriptor, memberSymbol);

        return true;
    }

    /// <summary>
    /// Returns <c>true</c> if the property's declared type is either a nullable reference type
    /// (e.g. <c>string?</c>) or a nullable value type (<c>int?</c> / <c>Nullable&lt;int&gt;</c>).
    /// </summary>
