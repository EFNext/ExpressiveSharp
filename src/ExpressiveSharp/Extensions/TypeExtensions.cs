...
public static class TypeExtensions
{
    ...
    private static bool MethodInfosEqual(MethodInfo? first, MethodInfo second)
    {
        ...
        var secondParams = second.GetParameters();
        ...
        var firstGenericArgs = first.GetGenericArguments();
        var secondGenericArgs = second.GetGenericArguments();
        if (firstGenericArgs.Length != secondGenericArgs.Length)
            return false;
        for (var i = 0; i < firstGenericArgs.Length; i++)
        {
            if (firstGenericArgs[i] != secondGenericArgs[i])
                return false;
        }
